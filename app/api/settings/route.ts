...
export async function POST(request: Request) {
  try {
    ...
    const { name, address, phone, email, website, logo, description, facebookUrl, twitterUrl, linkedinUrl, instagramUrl, workingHours, tvaRate, paymentTerms, deliveryTerms, legalNotice, cachetUrl, cachetFileName } = data
    ...
    let settings
    if (existingSettings) {
      // Use raw SQL for update with quoted camelCase identifiers matching Prisma schema
      settings = await prisma.$executeRaw`
        UPDATE company_settings SET 
          ...
          "cachetFileName" = ${cachetFileName || null},
          "updatedAt" = NOW()
      ...
      // Use raw SQL for create with quoted camelCase identifiers
      await prisma.$executeRaw`
        INSERT INTO company_settings (
          ...
          "tvaRate", "paymentTerms", "deliveryTerms", "legalNotice", "cachetUrl", "cachetFileName", "createdAt", "updatedAt"
        ) VALUES (
          'default',
          ${name || 'JetGlass'},
          ${address || ''},
